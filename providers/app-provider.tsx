...
export const [AppProvider, useApp] = createContextHook<AppState>(() => {
  ...
  const loadPersistedData = async () => {
    try {
      ...
      const [userData, easData, mtData, mt4Data, mt5Data, firstTimeData, activeSymbolsData, mt4SymbolsData, mt5SymbolsData, botActiveData, heroHiddenData] = await Promise.allSettled([
    ...
    try {
      ...
      if (active) {
        ...
        if (primaryEA && primaryEA.phoneSecretKey) {
          console.log('Starting EMC signals polling for EA:', primaryEA.licenseKey);
